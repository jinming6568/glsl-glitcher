...
export default async function () {
  ...
  filterArr.forEach(item => {
    ...
    console.log(item.glsl.program)
    let a_Position = gl.getAttribLocation(item.glsl.program, 'a_Position')
    ...
    item.controller = gui.add({[item.name]: item.set}, item.name, item.min, item.max, item.step)
