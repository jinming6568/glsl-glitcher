...
// a one-liner I found here and there on the web
...
void main() {
  ...
  vec3 inCol = texture2D(texture, uv).rgb;
  vec3 outCol = inCol;
  ...
  for (float i = 0.0; i < 10.0; i += 1.0) {
    ...
    if (insideRange(uv.y, sliceY, fract(sliceY+sliceH)) == 1.0 ){
      outCol = texture2D(texture, uvOff).rgb;
    }
  }
  ...
  if (rnd < 0.33){
    outCol.r = texture2D(texture, uvOff).r;
  }else if (rnd < 0.66){
    outCol.g = texture2D(texture, uvOff).g;
  } else{
    outCol.b = texture2D(texture, uvOff).b;
  }
